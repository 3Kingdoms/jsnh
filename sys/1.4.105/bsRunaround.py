...
class RunaroundGame(bs.CoopGameActivity):
    ...
    def __init__(self, settings={}):
        ...
        # jasonhu5
        # print(str(self._tntSpawnPosition))
        self._tntSpawnMaterial = bs.Material()
        self._tntSpawnMaterial.addActions(
            conditions=("theyHaveMaterial",bs.getSharedObject('playerMaterial')),
            actions=(("modifyPartCollision","collide",True),
                     ("modifyPartCollision","physical",False),
                     ("call","atConnect",self._handleReachedEndA)))

        self._spawnMaterial = bs.Material()
        self._spawnMaterial.addActions(
            conditions=("theyHaveMaterial",bs.getSharedObject('playerMaterial')),
            actions=(("modifyPartCollision","collide",True),
                     ("modifyPartCollision","physical",False),
                     ("call","atConnect",self._handleReachedEndB)))
        # 
    ...
    def onTransitionIn(self):
        ...
        # jasonhu5
        print(str(self.getMap().defs.boxes['scoreRegion'][0:3]))
        print(str(self._mapType.defs.points['tntLoc'][0:3]))
        print(str(self.getMap().defs.boxes['scoreRegion'][6:9]))
        self._tntSpawnPosition = bs.NodeActor(bs.newNode('region',
                                                    attrs={'position':self._mapType.defs.points['tntLoc'][0:3],
                                                           'scale':self.getMap().defs.boxes['scoreRegion'][6:9],
                                                           'type': 'box',
                                                           'materials':[self._tntSpawnMaterial]}))
        self._spawnPosition = bs.NodeActor(bs.newNode('region',
                                                    attrs={'position':self._mapType.defs.points['spawn1'][0:3],
                                                           'scale':self.getMap().defs.boxes['scoreRegion'][6:9],
                                                           'type': 'box',
                                                           'materials':[self._spawnMaterial]}))

        tntpos = self._mapType.defs.points['tntLoc'][0:3]
        self._spawnB = [tntpos[0] + 1.5, tntpos[1], tntpos[2]]
        self._spawnA = [self._spawnCenter[0] + 1.5,self._spawnCenter[1],self._spawnCenter[2]]
        self._spawnPos = self._spawnB
        # 
    ...
    def _handleReachedEndA(self):
        n = bs.getCollisionInfo("opposingNode")
        spaz = n.getDelegate()

        # bs.screenMessage("a")
        self._spawnPos = self._spawnA

        # jasonhu5
        # print(str(spaz))
        if isinstance(spaz, bs.PlayerSpaz):
            # bs.screenMessage("player!")
            spaz.handleMessage(bs.DieMessage(immediate=True))
            # spaz.handleMessage(bs.PlayerSpazDeathMessage(spaz, True, spaz, 'goal'))
    ...
    def spawnPlayer(self,player):
        ...
        # jasonhu5
        # pos = (self._spawnCenter[0],self._spawnCenter[1],self._spawnCenter[2] - 2)
        pos = self._spawnPos
        # 
    ...
    def _awardFlawlessBonus(self):
        ...
        self._score += self._flawlessBonus
        self._updateScores()
    ...
    def _updateScores(self):
        ...
        if self._preset == 'endless':
            if self._score >= 500: self._awardAchievement('Runaround Master')
            if self._score >= 1000: self._awardAchievement('Runaround Wizard')
            if self._score >= 2000: self._awardAchievement('Runaround God')
