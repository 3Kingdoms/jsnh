...
class Chooser(object):
    ...
    def _getName(self,full=False):
        ...
        if name == '_random':
            ...
            name = bs.Lstr(resource='createEditPlayerText',fallback='editProfileWindow.titleNewText').evaluate()
        else:
    ...
    def handleMessage(self,m):
        ...
        if isinstance(m,ChangeMessage):
            ...
            if m.what == 'team':
                ...
                self._selectedTeamIndex = (self._selectedTeamIndex + m.value) % len(self.getLobby()._teams)
                self._updateText()
                self.updatePosition()
                self._updateIcon()
                ...
                if len(self.profileNames) == 1:
                    ...
                    self.profileIndex = (self.profileIndex + m.value) % len(self.profileNames)
                    self.updateFromPlayerProfile()
                
            elif m.what == 'character':
                bs.playSound(self._clickSound)
                # update our index in our local list of characters
                self.characterIndex = (self.characterIndex + m.value) % len(self.characterNames)
                self._updateText()
                self._updateIcon()
        ...
        if self.getLobby()._useTeamColors:
            for i,teamRef in enumerate(self.getLobby()._teams):
                ...
                if i != self._selectedTeamIndex:
                    # find the dominant component this team's color
                    # and adjust ours so that component is not super-dominant
                    maxVal = 0
                    maxIndex = 0
                    for j in range(3):
...
class Lobby(object):
    ...
    def __init__(self):
        ...
        vOffset = -150 if isinstance(session,bs.CoopSession) else -30
