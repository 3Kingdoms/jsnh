...
class CTFFlag(bs.Flag):
    ...
    def resetReturnTimes(self):
        self._timeOutRespawnTime = int(self.getActivity().settings['Flag Idle Return Time'])
        self._touchReturnTime = float(self.getActivity().settings['Flag Touch Return Time'])
...
class CTFGame(bs.TeamGameActivity):
    ...
    def getSettings(cls,sessionType):
        return [("Score to Win",{'minValue':1,'default':3}),
                ("Flag Touch Return Time",{'minValue':0,'default':0,'increment':1}),
                ("Flag Idle Return Time",{'minValue':5,'default':30,'increment':5}),
    ...
    def getInstanceDescription(self):
        ...
        else: return ('Steal the enemy flag ${ARG1} times.',self.settings['Score to Win'])
    ...
    def onTeamJoin(self,team):
        ...
        team.gameData['baseRegion'] = bs.newNode("region",
        ...
        spazMatNoFlagPhysical = team.gameData['spazMaterialNoFlagPhysical'] = bs.Material()
        spazMatNoFlagCollide = team.gameData['spazMaterialNoFlagCollide'] = bs.Material()
        flagMat = team.gameData['flagMaterial'] = bs.Material()

        # some parts of our spazzes don't collide physically with our flags but generate callbacks
        spazMatNoFlagPhysical.addActions(conditions=('theyHaveMaterial',flagMat),
                                         actions=(('modifyPartCollision','physical',False),
                                                  ('call','atConnect',lambda: self._handleHitOwnFlag(team,1)),
                                                  ('call','atDisconnect',lambda: self._handleHitOwnFlag(team,0))))
        # other parts of our spazzes don't collide with our flags at all
        spazMatNoFlagCollide.addActions(conditions=('theyHaveMaterial',flagMat),
                                        actions=('modifyPartCollision','collide',False))

        # we wanna know when *any* flag enters/leaves our base
        baseRegionMat.addActions(conditions=('theyHaveMaterial',bs.Flag.getFactory().flagMaterial),
                                 actions=(('modifyPartCollision','collide',True),
                                          ('modifyPartCollision','physical',False),
                                          ('call','atConnect',lambda: self._handleFlagEnteredBase(team)),
                                          ('call','atDisconnect',lambda: self._handleFlagLeftBase(team))))
    ...
    def _handleFlagEnteredBase(self,team):
        ...
        if flag.getTeam() is team:
            ...
            if team.gameData['homeFlagAtBase']:
                ...
                try: player = flag._lastPlayerToHold
                except Exception: player = None
                if player is not None and player.exists() and player.getTeam() is team:
            ...
            # if the home-team flag isn't here, print a message to that effect
                ...
                if t - self._lastHomeFlagNoticePrintTime > 5000:
                    ...
                    tNode = bs.newNode('text',
                                       attrs={'text':bs.Lstr(resource='ownFlagAtYourBaseWarning'),
                                              'inWorld':True,
                                              'scale':0.013,
                                              'color':(1,1,0,1),
                                              'hAlign':'center',
                    ...
                    bs.animate(tNode,'scale',{0:0,200:0.013,4800:0.013,5000:0})
    ...
    def _tick(self):
        ...
        for team in self.teams:
            ...
            else: timeOutCountingDown = False
            ...
            if flag.node.exists() and flag._counter.exists():
                ...
                # to show its auto-return counter.  (if there's self-touches its showing that time)
                if team.gameData['flagReturnTouches'] == 0:
                    flag._counter.text = str(flag._timeOutRespawnTime) if (timeOutCountingDown and flag._timeOutRespawnTime <= 10) else ''
    ...
    def _handleFlagLeftBase(self,team):
        ...
        try: flag = opNode.getDelegate()
        except Exception: return # can happen when we kill a flag

        if flag.getTeam() is team:

            # check times here to prevent too much flashing
            if 'lastFlagLeaveTime' not in team.gameData or curTime - team.gameData['lastFlagLeaveTime'] > 3000:
                bs.playSound(self._alarmSound,position=team.gameData['basePos'])
                self._flashBase(team)
    ...
    def _touchReturnUpdate(self,team):
        ...
        if team.gameData['homeFlagAtBase'] or team.gameData['flag']._heldCount > 0:
            ...
            if team.gameData['touchReturnTimerTicking'] is None:
                team.gameData['touchReturnTimerTicking'] = bs.NodeActor(bs.newNode('sound',
                                                                                   attrs={'sound':self._tickingSound,
                                                                                          'positional':False,
                                                                                          'loop':True}))
        flag = team.gameData['flag']
    ...
    def _awardPlayersTouchingOwnFlag(self,team):
        for player in team.players:
            if player.gameData['touchingOwnFlag'] > 0:
                returnScore = 10 + 5 * int(self.settings['Flag Touch Return Time'])
                self.scoreSet.playerScored(player,returnScore,screenMessage=False)
    ...
    def spawnPlayerSpaz(self,*args,**keywds):
        ...
        noPhysicalMats = [spaz.getPlayer().getTeam().gameData['spazMaterialNoFlagPhysical']]
        noCollideMats = [spaz.getPlayer().getTeam().gameData['spazMaterialNoFlagCollide']]
        # our normal parts should still collide; just not physically (so we can calc restores)
        spaz.node.materials = list(spaz.node.materials)+noPhysicalMats
        spaz.node.rollerMaterials = list(spaz.node.rollerMaterials)+noPhysicalMats
        # pickups and punches shouldn't hit at all though
        spaz.node.punchMaterials = list(spaz.node.punchMaterials)+noCollideMats
        spaz.node.pickupMaterials = list(spaz.node.pickupMaterials)+noCollideMats
        spaz.node.extrasMaterials = list(spaz.node.extrasMaterials)+noCollideMats
        return spaz
