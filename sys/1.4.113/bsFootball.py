...
# use this to maintain the description and ids for corresponding tasks
gMissionKill = "Kill"
gMissionCarry = "Carry"
gMissionHold = "Hold"
gMissionDefs = {
    gMissionKill: "Kill enermies to score",
    gMissionCarry: "Carry the flag across to score",
    gMissionHold: "Carry flag for 15 sec to score",
}

# used to check if a mission ID is for which task
gMissionWithIDs = {}
# used to assign tasks to players
gMissionDict = {}

def _genGlobalMission():
    counter = 0
    for _, val in gMissionRealDict.items():
        for i in range(val.get("number")):
            gMissionDict[counter] = val.get("description")
            print(counter)
...
class FootballTeamGame(bs.TeamGameActivity):
    ...
    def _handleKillScore(self, player, killer):
        ...
        # handle team-kills
        if not killer.getTeam() is player.getTeam():
            self._handleRealScore(killer, killer.getTeam())
     
    def _handleHoldScore(self, player):
        self._timer = bs.Timer(1000,bs.WeakCall(self._update),repeat=True)

    def _handleRealScore(self, player, team):
        team.gameData['score'] += 7
